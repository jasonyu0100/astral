...
export function SpacesMapContentsSceneConnections() {
  ...
  const maxWeight = spaceIdeaRelationshipListController.state.objs.reduce(
    (max, relationship) =>
      relationship.weight > max ? relationship.weight : max,
    0,
  );
  const minWeight = spaceIdeaRelationshipListController.state.objs.reduce(
    (min, relationship) =>
  ...
  return (
    <svg className='h-full w-full'>
      {connectionMode == SpacesMapConnectionMode.DEFAULT && (
        <>
          {spaceIdeaRelationshipListController.state.objs.map(
            (relationship) => {
