...
import { SpacesMissionSidebarBacklogAdd } from './add/main';
import { SpacesMissionBacklogItem } from './item/main';

export function SpacesMissionSidebarBacklog() {
        ...
        <GlassWindowContents className='flex h-full w-full flex-col space-y-[1rem] p-[1rem]'>
          {backlog.map((task) => (
            <ContextForTaskObj.Provider value={task}>
              <SpacesMissionBacklogItem />
            </ContextForTaskObj.Provider>
          ))}
          <HorizontalDivider />
          <SpacesMissionSidebarBacklogAdd />
        </GlassWindowContents>
      </GlassWindowFrame>
    </div>
  );
