...
export function SpacesChannelInputText() {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
      spacesChannelController.actions.sendMessageToConversation();
    }
  };
    ...
    >
      <GlassWindowContents className='flex w-full flex-row items-center space-x-[1rem] pl-[1rem] pr-[1rem]'>
        <div
          ...
          onClick={() =>
            spacesChannelController.actions.updateSidebarContentMode(
              SpacesChannelSidebarContentMode.MEMBERS,
            )
          }
        >
          <UserDisplayPictureElement
            fileElem={spacesChannelController.state.selectedUser?.dp}
          />
        </div>
        <input
