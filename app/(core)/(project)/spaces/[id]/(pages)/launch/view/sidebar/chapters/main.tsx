...
import { SpacesLaunchSidebarChaptersAdd } from './add/main';
import { SpacesLaunchSidebarChaptersChapter } from './item/main';

export function SpacesLaunchSidebarChapters() {
        ...
        <GlassWindowContents className='flex h-full w-full flex-col space-y-[1rem] p-[1rem]'>
          {chapterListController.state.objs.map((chapter) => (
            <ContextForSpaceChapterObj.Provider value={chapter}>
              <SpacesLaunchSidebarChaptersChapter />
            </ContextForSpaceChapterObj.Provider>
          ))}
          <HorizontalDivider />
          <SpacesLaunchSidebarChaptersAdd />
        </GlassWindowContents>
      </GlassWindowFrame>
    </div>
  );
