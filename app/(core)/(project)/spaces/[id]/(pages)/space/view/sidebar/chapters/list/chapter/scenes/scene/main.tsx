...
import SpacesSpaceSidebarSceneIndicator from './indicator/main';

export function SpacesSpaceChapterScene() {
      ...
      {active ? (
        <div className='flex w-full animate-pulse-slow flex-row items-center justify-between'>
          <div className='flex w-full flex-col'>
            <p className='w-full text-lg font-light text-slate-300'>
              {index + 1}. {scene.title}
            </p>
          </div>
          <SpacesSpaceSidebarSceneIndicator />
        </div>
      ) : (
