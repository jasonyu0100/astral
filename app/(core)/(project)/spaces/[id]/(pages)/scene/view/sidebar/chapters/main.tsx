...
import { SpacesSceneSidebarChaptersAdd } from './add/main';
import { SpacesSceneSidebarChaptersChapter } from './item/main';

export function SpacesSceneSidebarChapters() {
        ...
        <GlassWindowContents className='flex h-full w-full flex-col space-y-[1rem] p-[1rem]'>
          {chapterListController.state.objs.map((chapter) => (
            <ContextForSpaceChapterObj.Provider value={chapter}>
              <SpacesSceneSidebarChaptersChapter />
            </ContextForSpaceChapterObj.Provider>
          ))}
          <HorizontalDivider />
          <SpacesSceneSidebarChaptersAdd />
        </GlassWindowContents>
      </GlassWindowFrame>
    </div>
  );
