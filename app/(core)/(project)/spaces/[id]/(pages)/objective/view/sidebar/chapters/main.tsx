...
import { SpacesObjectiveSidebarChaptersAdd } from './add/main';
import { SpacesObjectiveSidebarChaptersChapter } from './item/main';

export function SpacesObjectiveSidebarChapters() {
        ...
        <GlassWindowContents className='flex h-full w-full flex-col space-y-[1rem] p-[1rem]'>
          {chapterListController.state.objs.map((chapter) => (
            <ContextForSpaceChapterObj.Provider value={chapter}>
              <SpacesObjectiveSidebarChaptersChapter />
            </ContextForSpaceChapterObj.Provider>
          ))}
          <HorizontalDivider />
          <SpacesObjectiveSidebarChaptersAdd />
        </GlassWindowContents>
      </GlassWindowFrame>
    </div>
  );
