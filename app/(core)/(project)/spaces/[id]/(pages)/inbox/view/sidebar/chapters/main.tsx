...
import { SpaceInboxSidebarChaptersAdd } from './add/main';
import { SpaceInboxSidebarChaptersChapter } from './item/main';

export function SpaceInboxSidebarChapters() {
        ...
        <GlassWindowContents className='flex h-full w-full flex-col space-y-[1rem] p-[1rem]'>
          {chapterListController.state.objs.map((chapter) => (
            <ContextForSpaceChapterObj.Provider value={chapter}>
              <SpaceInboxSidebarChaptersChapter />
            </ContextForSpaceChapterObj.Provider>
          ))}
          <HorizontalDivider />
          <SpaceInboxSidebarChaptersAdd />
        </GlassWindowContents>
      </GlassWindowFrame>
    </div>
  );
