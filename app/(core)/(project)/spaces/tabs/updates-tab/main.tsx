...
export function SpacesTabUpdates({ tab }: { tab?: string }) {
  ...
  const url = spaceId
    ? `${spacesMap.spaces.id.updates.link(spaceId)}?chapter=${chapterId ?? ''}&post=${postId ?? ''}`
