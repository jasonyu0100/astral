...
import { BoardModalType } from '@/(core)/(project)/space/[id]/(pages)/board/page';

export function BoardHeaderDefaultBoardButton({ ...props }: ButtonInputProps) {
  return (
    <button
      className='flex h-[4rem] w-full flex-row items-center hover:bg-slate-900'
      {...props}
    >
      ...
      <div className='flex h-[4rem] flex-grow flex-row items-center pl-[10px]'>
        <p className='font-bold text-slate-300'>{BoardModalType.DEFAULT}</p>
      </div>
    </button>
  );
}
