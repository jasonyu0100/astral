...
export function PublicSpaceTasksSectionDone() {
    ...
    <div className='flex flex-col space-y-[1rem]'>
      ...
      >
        <div className='flex flex-row items-center space-x-[1rem]'>
          <div className='flex h-[1.5rem] w-[1.5rem] flex-shrink-0 flex-col items-center justify-center rounded-full bg-blue-500'>
            <p className='text-md font-bold text-white'>{done.length}</p>
          </div>
          <p className='text-2xl font-bold text-slate-300'>{TaskStatus.DONE}</p>
        </div>
      ...
      {togglableController.toggled && (
        <div className='flex w-full flex-shrink-0 flex-col space-y-[1rem] py-[1rem]'>
          {done.length === 0 && (
            <p className='text-slate-300'>No tasks in done</p>
          )}
          {done.map((task) => (
