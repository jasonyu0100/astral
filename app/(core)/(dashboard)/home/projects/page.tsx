...
import {
  ContextForHomeProjects,
  useControllerForHomeProjects,
} from './controller/main';
import {
  ContextForHomeProjectsModals,
  HomeProjectsModals,
} from './modal/controller/main';
import {
  ContextForHomeProjectsCreateSpace,
  useControllerForHomeProjectsCreateSpace,
} from './modal/create-space/controller/main';
import { HomeProjectsView } from './view/view';
...
function ModalWrapper({ children }: { children: React.ReactNode }) {
  return <HomeProjectsModals>{children}</HomeProjectsModals>;
}

function ControllerWrapper({ children }: { children: React.ReactNode }) {
  const createSpaceController = useControllerForHomeProjectsCreateSpace();
  const homePersonalController = useControllerForHomeProjects();

  return (
    <>
      <ContextForHomeProjects.Provider value={homePersonalController}>
        <ContextForHomeProjectsCreateSpace.Provider
          value={createSpaceController}
        >
          {children}
        </ContextForHomeProjectsCreateSpace.Provider>
      </ContextForHomeProjects.Provider>
    </>
  );
}
