...
import {
  ContextForCreateSpaces,
  useControllerForCreateSpaces,
} from './controller/main';
import {
  ContextForCreateSpacesModals,
  CreateSpacesModals,
} from './modal/controller/main';
import {
  ContextForCreateSpacesCreateSpace,
  useControllerForCreateSpacesCreateSpace,
} from './modal/create-space/controller/main';
import { CreateSpacesView } from './view/view';
...
function ModalWrapper({ children }: { children: React.ReactNode }) {
  return <CreateSpacesModals>{children}</CreateSpacesModals>;
}

function ControllerWrapper({ children }: { children: React.ReactNode }) {
  const createSpaceController = useControllerForCreateSpacesCreateSpace();
  const homePersonalController = useControllerForCreateSpaces();

  return (
    <>
      <ContextForCreateSpaces.Provider value={homePersonalController}>
        <ContextForCreateSpacesCreateSpace.Provider
          value={createSpaceController}
        >
          {children}
        </ContextForCreateSpacesCreateSpace.Provider>
      </ContextForCreateSpaces.Provider>
    </>
  );
}
