import { DraftHeaderNetworkBoardButton } from './network/main';
import { DraftHeaderInfoBoardButton } from './info/main';
import { DraftHeaderVisualBoardButton } from './visual/main';
import { DraftHeaderSoundBoardButton } from './sound/main';
import {
  DraftContext,
  DraftModalType,
} from '@/(cx)/(process)/space/[id]/(stages)/draft/page';
import { useContext, useState } from 'react';

export function DraftHeaderToggleButton() {
  const { modalType, updateModalType } = useContext(DraftContext);
  const [showList, changeShowList] = useState(false);

  return (
    <div className='relative'>
      <button
        className={
          'flex h-[60px] w-[60px] flex-col items-center justify-center border-r  border-slate-500 border-opacity-30'
        }
        onClick={() => changeShowList(!showList)}
      >
        <svg
          xmlns='http://www.w3.org/2000/svg'
          className='h-1/2 w-1/2'
          viewBox='0 0 24 24'
          fill='none'
        >
          <mask
            id='mask0_3152_44'
            maskUnits='userSpaceOnUse'
            x='0'
            y='0'
            width='24'
            height='24'
          >
            <rect width='24' height='24' fill='#D9D9D9' />
          </mask>
          <g mask='url(#mask0_3152_44)'>
            <path
              d='M5.5 22C4.53333 22 3.70833 21.6583 3.025 20.975C2.34167 20.2917 2 19.4667 2 18.5C2 17.5333 2.34167 16.7083 3.025 16.025C3.70833 15.3417 4.53333 15 5.5 15C5.8 15 6.09167 15.0375 6.375 15.1125C6.65833 15.1875 6.925 15.2917 7.175 15.425L11 11.6V8.85C10.2667 8.63333 9.66667 8.22083 9.2 7.6125C8.73333 7.00417 8.5 6.3 8.5 5.5C8.5 4.53333 8.84167 3.70833 9.525 3.025C10.2083 2.34167 11.0333 2 12 2C12.9667 2 13.7917 2.34167 14.475 3.025C15.1583 3.70833 15.5 4.53333 15.5 5.5C15.5 6.3 15.2667 7.00417 14.8 7.6125C14.3333 8.22083 13.7333 8.63333 13 8.85V11.6L16.85 15.425C17.1 15.2917 17.3625 15.1875 17.6375 15.1125C17.9125 15.0375 18.2 15 18.5 15C19.4667 15 20.2917 15.3417 20.975 16.025C21.6583 16.7083 22 17.5333 22 18.5C22 19.4667 21.6583 20.2917 20.975 20.975C20.2917 21.6583 19.4667 22 18.5 22C17.5333 22 16.7083 21.6583 16.025 20.975C15.3417 20.2917 15 19.4667 15 18.5C15 18.2 15.0375 17.9083 15.1125 17.625C15.1875 17.3417 15.2917 17.075 15.425 16.825L12 13.4L8.575 16.825C8.70833 17.075 8.8125 17.3417 8.8875 17.625C8.9625 17.9083 9 18.2 9 18.5C9 19.4667 8.65833 20.2917 7.975 20.975C7.29167 21.6583 6.46667 22 5.5 22ZM18.5 20C18.9167 20 19.2708 19.8542 19.5625 19.5625C19.8542 19.2708 20 18.9167 20 18.5C20 18.0833 19.8542 17.7292 19.5625 17.4375C19.2708 17.1458 18.9167 17 18.5 17C18.0833 17 17.7292 17.1458 17.4375 17.4375C17.1458 17.7292 17 18.0833 17 18.5C17 18.9167 17.1458 19.2708 17.4375 19.5625C17.7292 19.8542 18.0833 20 18.5 20ZM12 7C12.4167 7 12.7708 6.85417 13.0625 6.5625C13.3542 6.27083 13.5 5.91667 13.5 5.5C13.5 5.08333 13.3542 4.72917 13.0625 4.4375C12.7708 4.14583 12.4167 4 12 4C11.5833 4 11.2292 4.14583 10.9375 4.4375C10.6458 4.72917 10.5 5.08333 10.5 5.5C10.5 5.91667 10.6458 6.27083 10.9375 6.5625C11.2292 6.85417 11.5833 7 12 7ZM5.5 20C5.91667 20 6.27083 19.8542 6.5625 19.5625C6.85417 19.2708 7 18.9167 7 18.5C7 18.0833 6.85417 17.7292 6.5625 17.4375C6.27083 17.1458 5.91667 17 5.5 17C5.08333 17 4.72917 17.1458 4.4375 17.4375C4.14583 17.7292 4 18.0833 4 18.5C4 18.9167 4.14583 19.2708 4.4375 19.5625C4.72917 19.8542 5.08333 20 5.5 20Z'
              fill='#CBD5E1'
            />
          </g>
        </svg>
      </button>
      {showList && (
        <div className='absolute top-[60px] z-50 flex w-[300px] flex-col border border-slate-500 border-opacity-30 bg-slate-800 p-[0px]'>
          {modalType !== DraftModalType.DEFAULT && (
            <DraftHeaderNetworkBoardButton
              onClick={() => updateModalType(DraftModalType.DEFAULT)}
            />
          )}
          {modalType !== DraftModalType.SOUND && (
            <DraftHeaderSoundBoardButton
              onClick={() => updateModalType(DraftModalType.SOUND)}
            />
          )}
          {modalType !== DraftModalType.VISUAL && (
            <DraftHeaderVisualBoardButton
              onClick={() => updateModalType(DraftModalType.VISUAL)}
            />
          )}
          {modalType !== DraftModalType.INFO && (
            <DraftHeaderInfoBoardButton
              onClick={() => updateModalType(DraftModalType.INFO)}
            />
          )}
        </div>
      )}
    </div>
  );
}
