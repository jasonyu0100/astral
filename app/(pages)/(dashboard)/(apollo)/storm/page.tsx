...
export default function Page() {
  const [messages, changeMessages] = useState<StormMessageObj[]>(
    stormModel.points.point.chat.messages.example
  );
  
  const sendMessage = (message: string) => {
    changeMessages((prev) => [...prev, stormModel.points.point.chat.messages.message.example]);
    changeMessages((prev) => [...prev, stormModel.points.point.chat.messages.message.example]);
    alert(`Sent Message ${message}`);
  };
              

  return <StormView messages={messages} sendMessage={sendMessage} />;
}
