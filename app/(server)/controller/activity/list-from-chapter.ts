...
import {
  UserActivityObj,
  UserActivityVariant,
} from '@/(server)/model/activity/main';
import { createContext, useMemo, useState } from 'react';

type TargetObj = UserActivityObj;
const gqlDbWrapper = userActivityDbWrapper;
...
const useControllerForUserActivityListFromChapter = (
  listId: string | boolean | number,
  ...
  const createActions: CreateActions = {
    ...
    createFromChapter: async (userId, spaceId, chapterId) => {
      const createObj: Omit<TargetObj, 'id'> = {
        ...
        spaceId: spaceId,
        created: new Date().toISOString(),
        variant: UserActivityVariant.CHAPTER,
      };
      const newObj = await gqlDbWrapper.createObj(createObj);
      const newObjs = stateActions.pushBack(newObj);
      stateActions.searchAndUpdateQuery(query, newObjs);
    ...
    createFromChapterScene: async (userId, spaceId, chapterId, sceneId) => {
      const createObj: Omit<TargetObj, 'id'> = {
        ...
        description: '',
        created: new Date().toISOString(),
        variant: UserActivityVariant.SCENE,
      };
      const newObj = await gqlDbWrapper.createObj(createObj);
      const newObjs = stateActions.pushBack(newObj);
      stateActions.searchAndUpdateQuery(query, newObjs);
    ...
    createFromChapterPost: async (userId, spaceId, chapterId, postId) => {
      const createObj: Omit<TargetObj, 'id'> = {
        userId: userId,
        chapterId: chapterId,
        postId: postId,
...
const ContextForUserActivityListFromChapter = createContext({} as Controller);
export {
  ContextForUserActivityListFromChapter as ContextForUserActivityListFromChapter,
  useControllerForUserActivityListFromChapter as useControllerForUserActivityListFromChapter,
};
