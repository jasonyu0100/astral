...
import { postLinkModel, PostLinkObj } from '@/(server)/model/post/link/main';
import { createContext, useMemo, useState } from 'react';

type TargetObj = PostLinkObj;
const gqlDbWrapper = postLinkDbWrapper;
const listIdKey = postLinkModel.parentKey;
...
const useControllerForPostLinkList = (
  listId: string | boolean | number,
  ...
  const createActions: CreateActions = {
    ...
    createFromLog: async (userId: string, postId: string, logId: string) => {
      const createObj: Omit<TargetObj, 'id'> = {
        created: new Date().toISOString(),
        userId: userId,
        postId: postId,
...
const ContextForPostLinkList = createContext({} as Controller);
export { ContextForPostLinkList, useControllerForPostLinkList };
